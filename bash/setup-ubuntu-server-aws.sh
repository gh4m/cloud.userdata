...
WG_CLOUDVPN_ETH_NAMESERVER_IP_ADDR=169.254.169.123
...
echo "${WG_CLOUDVPN_PRIVATE_IP_ADDR} ${WG_CLOUDVPN_WGS1_FQDN}" >> /etc/hosts
## home host setup
HOME_ISP_PUBLIC_IP_ADDR=$(dig +short ${WG_HOMEISP_NIC_FQDN} | tail -n1 | grep -E -o "^([0-9]{1,3}[\.]){3}[0-9]{1,3}$")
echo "${HOME_ISP_PUBLIC_IP_ADDR} ${WG_HOMEISP_NIC_FQDN}" >> /etc/hosts

timedatectl set-timezone America/New_York

sed -i "/pool ntp.ubuntu.com/c\server ${WG_CLOUDVPN_ETH_NAMESERVER_IP_ADDR} prefer iburst minpoll 4 maxpoll 4" /etc/chrony/chrony.conf
