...
then
  ...
  $ufw_cmd status | grep "\s${home_cidr_previous_value}\s*$" | egrep '^22/tcp' | while read rn
  do
  ...
  do
    ...
    $ufw_cmd        $ufw_allowdeny out proto $ufw_proto to ${home_ip_just_dug_up}      port $ufw_port
  done
fi
